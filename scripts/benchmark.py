...
while True:
    ...
    cv2.putText(output, f"Pose: {pose_latency:.1f} ms",
    ...
    cv2.putText(output, "Press Q to finish benchmark", (20, 145),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

    cv2.imshow("Laptop Benchmark", output)
...
avg_classifier = np.mean(classifier_latencies)
...
file_path = benchmark_dir / "laptop_mx330.csv"
...
with open(file_path, "a", newline="") as file:
    ...
    if not file_exists:
        writer.writerow(["date", "device", "pose_model", "classifier",
                        "frames", "avg_fps", "avg_pose_ms", "avg_classifier_ms"])

    writer.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "NVIDIA GeForce MX330", "YOLOv8n-pose",
                    "Random Forest", len(fps_values), round(avg_fps, 2), round(avg_pose, 2), round(avg_classifier, 2)])
